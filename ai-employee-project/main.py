...
class Orchestrator:
    ...
    def _refresh_dashboard(self) -> None:
        try:
            ...
            from helpers.inbox_processor import escalate_overdue
            escalate_overdue(self.vault_path, threshold_hours=24)
    ...
    def _health_check(self) -> None:
        ...
        for wt in self._watchers:
            ...
            self.logger.info(
                f"  {wt.name:<15} {status:<6}  restarts={restarts}  started={since}"
            )
        ...
        self.logger.info(f"  {'All watchers healthy.' if all_ok else 'Restart(s) triggered.'}")
        self.logger.info("──────────────────────────────────────────────────")
